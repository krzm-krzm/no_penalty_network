...
def cal_kakuritsu(now_location, idou_list, idou_probability):
    ...
    for i in range(len(idou_probability)):
        if noriori[idou_list[i]]==-1:
            ...
            p = (idou_probability[i] ** alpha) * ((Q / Distance[idou_list[i]][now_location[0]]) ** beta) * ((1 / (l[idou_list[i]] - now_location[1])) ** ganma) * (pheromon[now_location[0]][idou_list[i]]) ** delta #現在の時刻からの∆がヒューリスティック値
            #p = ((Q / Distance[idou_list[i]][now_location[0]]) ** beta) * (1 / (l[idou_list[i]] - now_location[0])) ** ganma
            kakuritsu_list.append(p)
            sum +=p
        else:
            p = (idou_probability[i] ** alpha) * (Q / Distance[idou_list[i]][now_location[0]]) ** beta * ((1 / (l[idou_list[i]] - now_location[1])) ** theta) * (pheromon[now_location[0]][idou_list[i]])**delta
...
#-----------------------------------------------------------------
    ...
    while True:
        ...
        for syaryo_number in range(Syaryo) :
            while True:
